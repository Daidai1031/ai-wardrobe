...
Next.js 16 App Router app ("ai-wardrobe") using Server Components + Route Handlers, Supabase (auth/DB/storage), Codex Vision for classification, and fal.ai for background removal. It replaces an earlier Python prototype — this is a from-scratch rewrite, not an upgrade.
...
1. Client already uploaded the original image to Supabase Storage (`wardrobe` bucket) and passes `{ originalUrl, storagePath, mode }`. `mode` is `"single" | "multi"`, set by a toggle in `UploadZone` (`src/components/closet/upload-zone.tsx`) that the user picks *before* uploading — defaults to `"single"`.
   ...
   - `mode: "multi"` still calls `detectItems()` (still needed for concrete SAM prompts) but trusts the user's claim over Codex's count guess: `itemCount = Math.max(2, detection.count)`.
   - `mode` omitted (older client) falls back to the original auto-detect behavior below, for backward compatibility.
2. `detectItems()` (`src/lib/ai/segment.ts`) — only called when `mode` isn't `"single"` — makes a Codex Haiku vision call asking for both the distinct-item count and concrete English object nouns for SAM 3.1. Before the `mode` toggle existed, this ran unconditionally on every upload; see `checklist.md` for why running SAM first instead would cost money on every single-item photo just to find out it's one item.
...
5. `classifyItem()` (`src/lib/ai/classify.ts`) downsizes with Sharp (max 1024px, JPEG q85 — Codex rejects images >10MB, and smaller images cost fewer tokens); `resizeForClassification()` is exported and reused by `detectItems()` too.
...
`POST /api/ai/stylist` (`src/app/api/ai/stylist/route.ts`) fetches the user's full active (`archived = false`) wardrobe and profile, inlines a compact JSON summary of both into the system prompt, and asks Codex to recommend outfits using only owned items. It is a single-turn call (no server-side conversation history) — the client is responsible for any chat history it wants to resend.
...
`checklist.md` tracks feature status per phase and a running debug log — check it before assuming a feature is unimplemented or before re-solving a previously-fixed build error (e.g. Next 16 `proxy.ts` rename, Sharp image resizing for Codex's 10MB limit, HEIC conversion). Multi-item SAM 3.1 segmentation is live-verified; the remaining upload UX gap is that there is no frontend checkbox UI to deselect individual detected items before they're all auto-classified. The outfit create/save drag-and-drop builder is implemented, including its freeform transparent Canvas, but exact Canvas geometry is not yet persisted for edit/reopen — and there is no edit entry point at all yet for an already-saved outfit in the `/outfits` library. `POST /api/ai/stylist` still only returns plain text (`{ reply }`); it has no structured/Canvas output and no way for the user to edit a recommended look, unlike the `/outfits` builder. There is no dashboard home page yet (`/` just redirects to `/closet` or `/login`) — daily recommendations combining Google Calendar and OpenWeather are planned there; the stylist route already has unused `context.weather`/`context.calendar` slots waiting for that data. Also incomplete: Google Calendar integration (no token storage in the schema yet), travel/capsule wardrobe features (placeholder pages only).
