...
| **轮换频率由用户决定** | ✅ 2026-08-13 已完成。口径改为「滚动 7 天内最多几天」（`MAX_WEAR_DAYS_BY_CATEGORY` + `profiles.rotation_limits`），入口是 `/plan` 的「Repeat rules」按钮。同批修掉了「总是那几件」的四个成因和「运动装进正式场合」。偏离原方案的两点见第三节「并行小需求 B」 |
...
#### B. 轮换频率由用户决定（Phase 6.2 收尾）— ✅ 2026-08-13 已实现

**⚠️ 落地时和下面的方案有两处刻意的偏离，以实际代码为准：**

1. **口径从「最小间隔天数」改成「滚动 7 天内最多几天」**，字段名也随之从 `repeat_gap_overrides` 改成 `rotation_limits`（`MAX_WEAR_DAYS_BY_CATEGORY` / `ROTATION_WINDOW_DAYS`）。原因是用户提的原话就是「同一双鞋一周内不重复超过两天（不是两次）」——而「间隔 2 天」实际允许周一/周三/周五三天，差的正是被投诉的那个行为。而且按天数说话，设置项和保证的是同一句话。取值 1–7，7 = 不限制（0 不再有意义，被拒绝）。默认值：上装/下装/连衣裙/外套 1 天、鞋 2 天、包和配饰 3 天（**包和配饰不再豁免**——豁免是「同一只 clutch 每天出现」的直接原因）。
2. **设置入口放在 `/plan` 的「Repeat rules」按钮，不是 `/profile`。** 用户明确要求，而且理由成立：这个问题是在看着这一周的时候产生的，把人支到另一个页面就丢了。这是唯一一个不在 `/profile` 编辑的 `profiles` 列。

同一批改动还顺带修了「总是推荐那几件」的另外三个原因（daily 完全不跑轮换、轮换看不到相邻天的历史、候选打分完全确定性），以及新增「运动装不进正式场合」规则。完整记录见 `checklist.md` Debug Log #19 / #20。
