...
| 25 | 点「连接 Google Calendar」跳到 Google 后报 `Error 401: invalid_client — The OAuth client was not found`（2026-08-15） | 这句话读起来像「Cloud Console 里的 OAuth client 被删了」，实际不是：`.env.local` 里的 client id/secret 是好的——拿它去 `oauth2.googleapis.com/token` 换一个假 code，Google 回的是 `invalid_grant: Malformed auth code`（说明 client 认证已经通过），拿它去 authorize 端点也正常跳到登录页。真正的原因是 `buildGoogleAuthUrl` 里写的是 `process.env.GOOGLE_CLIENT_ID!`，**变量没设时 `URLSearchParams` 会把字面量字符串 `"undefined"` 拼进 URL**，Google 对 `client_id=undefined` 的回答一字不差就是这句 401。已用 `client_id=undefined` 复现出完全相同的错误页。也就是说：出错的那次请求所在的进程没有 `GOOGLE_CLIENT_ID`——生产（Vercel）没配这两个环境变量，或者本地 dev server 是在写进 `.env.local` 之前起的 | 环境侧：在 Vercel 项目里补 `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` 后重新部署（本地则重启 dev server）。代码侧：`src/lib/google/client.ts` 新增 `requireGoogleEnv()`，三个调用点（authorize / code 交换 / refresh）都改走它，缺变量直接抛而不是把 `"undefined"` 发给 Google；`/api/google/auth` 捕获后返回 500 `Google Calendar OAuth is not configured on this deployment`，不再把用户丢到 Google 的错误页。**顺带查出的第二个问题**：这个 OAuth client 登记的 redirect URI 只有 `http://localhost:3000/api/google/callback` 和 `https://closet.daidingrdesigns.com/api/google/callback`——`redirectUri` 是从 `request.url` 推的，所以从 `ai-wardrobe-six.vercel.app` 或任何 `*-git-main-*.vercel.app` 预览域名点连接，即使环境变量补齐了也会换成 `Error 400: redirect_uri_mismatch`（已逐个探测确认） |
