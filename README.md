...
For an **existing** database, don't re-run the whole file — run only the re-runnable blocks you're missing: section 15 (planning tables + RPCs), section 16 (Human Stylist bookings), section 17 (`wardrobe_item_photos`, extra item angles), section 18 (stylist reviews), section 19 (`profiles.rotation_limits`, the per-category repeat rules edited from `/plan`), section 20 (Saved-Look reuse in plans). There is no migration tooling here, so a schema change only reaches the database when someone pastes it in. Symptom of a skipped block: the feature fails at runtime with PostgREST `PGRST205` — `Could not find the table 'public.<name>' in the schema cache` — while `npm run build` still passes. Section 19 is the one exception, on purpose: `rotation_limits` is read by its own small query (`readRotationLimits()`), so if the column isn't there yet the read fails quietly and planning continues on the built-in defaults rather than taking `/home` and `/plan` down. The only visible symptom is that the "Repeat rules" panel in `/plan` won't save. If the table exists and you still get `PGRST205`, PostgREST's cache is stale; run `notify pgrst, 'reload schema';`.
